...
function countVowels(str) {
  return str.split('').filter((el) => el.toLowerCase().match(/^[aeyiuo]+$/))
    .length;
}
...
function encodeToRot13(str) {
  ...
  return str
    ...
    .map((el) => {
      ...
      if (arr.indexOf(el) + 13 > arr.length - 1) {
        if (el === el.toUpperCase()) {
          return arr[arr.indexOf(el) + 13 - 26].toUpperCase();
        }
        return arr[arr.indexOf(el) + 13 - 26].toLowerCase();
      }
      if (el === el.toUpperCase()) {
        return arr[arr.indexOf(el) + 13].toUpperCase();
      }
      return arr[arr.indexOf(el) + 13].toLowerCase();
    })
    .join('');
}
